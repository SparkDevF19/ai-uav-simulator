...
# Directory to save non-collision images (Safe)
tmp_dir_safe = r"C:\Users\Not Orson\Pictures\Collision_Images\Safe"

# Directory to save collision images (Unsafe)
tmp_dir_unsafe = r"C:\Users\Not Orson\Pictures\Collision_Images\Unsafe"

try:
    os.makedirs(tmp_dir_safe)
    os.makedirs(tmp_dir_unsafe)
except OSError:
    if not os.path.isdir(tmp_dir_safe):
        raise
    if not os.path.isdir(tmp_dir_unsafe):
        raise
            ...
            if len(imagequeue) == QUEUESIZE:
                ...
                if collision_info.has_collided:
                    for i in range(QUEUESIZE):
                        filename = os.path.join(tmp_dir_unsafe, str(i + imageBatch))
                        airsim.write_file(os.path.normpath(filename + '.png'),imagequeue[i])
                    print("Writing to unsafe")
                    
                # if drone doesn't colide
                else:
                    for i in range(QUEUESIZE):
                        filename = os.path.join(tmp_dir_safe, str(i + imageBatch))
                        airsim.write_file(os.path.normpath(filename + '.png'),imagequeue[i])
